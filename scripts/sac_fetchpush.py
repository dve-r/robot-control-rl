...
class Args:
    ...
    """whether to randomize physics parameters each episode (mass, friction, size)"""
...
if __name__ == "__main__":
    ...
    if args.randomize:
        ...
        env_kwargs["mass_range"] = [0.5, 2.0]
        env_kwargs["friction_range"] = [0.5, 2.0]
        env_kwargs["size_range"] = [0.8, 1.2]
    ...
    if args.her:
        ...
        rb = HERReplayBuffer(
            ...
            goal_dim=3, # Goal represented as 3D position. Flattened obs.
    ...
    for global_step in range(args.total_timesteps):
        ...
        if args.her:
            ...
            episode_buffer["obs"].append(obs[0])
            episode_buffer["action"].append(actions[0])
            episode_buffer["next_obs"].append(real_next_obs[0])
            episode_buffer["reward"].append(rewards[0])
            episode_buffer["done"].append(terminations[0]) # 'or truncations[0]' was saving the wrong done value and using it in the Bellman Equation
            
            # On episode end (termination or truncation)
            if terminations[0] or truncations[0]:
                ...
                # Reset episode_buffer for the next episode.
                episode_buffer = {
